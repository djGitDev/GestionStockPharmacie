...
public:
    ...
    void inserer_arbre(const ArbreAvl<T> &arbre_autre);
    const T* rechercher(const T& element)const;
    bool contient(const T& element)const;


    void parcourirEnOrdre(std::ostream &os) ;
...
private:
    ...
    struct Noeud
    {
        ...
        Noeud* gauche;
        Noeud* droite;
        Noeud(const T& element) : contenu(element), gauche(nullptr), droite(nullptr) {}
    ...
    const T *rechercher(Noeud *noeud, const T &element) ;
    void inserer(Noeud* &noeud, const T &element);
    void vider(Noeud *noeud);
    void copier(const ArbreAvl<T> &autre, Noeud *&noeud);
    void inserer_arbre(Noeud* const &racine_autre);
    void parcourirEnOrdre(Noeud* n, std::ostream &os) ;
